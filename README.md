...
* **Box-Cox rejected on evidence.** λ ≈ -0.04 on the shifted series amplifies the single most-negative-price day into a >4σ outlier. Ljung-Box on transformed residuals is uniformly worse than raw (290 / 257 / 244 vs. 27.4 / 134 / 169 across the three model families).
* **Prediction interval calibration is the decisive selection criterion.** Auto-ARIMA's 30-day-plus PIs stay bounded around [-100, +280] EUR/MWh, whereas the manual airline and ETS PIs explode to ±1,000 EUR/MWh under compounding forecast variance from dual differencing and additive trend. On rolling-origin CV the three univariate models are tied on MASE, but auto-ARIMA's intervals are the only ones that remain operationally usable.
* **Wind validates merit-order theory but does not enhance forecasts.** The dynamic-regression wind coefficient is -9 × 10⁻⁴ EUR/MWh per MWh, right-signed and significant. For a typical median wind day (~21 GWh) that translates to about -19 EUR/MWh on price; for a high-wind day (Q3 ≈ 40 GWh), about -36 EUR/MWh. The economic effect is real; the forecasting gain over univariate auto-ARIMA is not.
